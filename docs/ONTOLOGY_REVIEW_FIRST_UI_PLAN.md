...
## 11. 补记：机器判定的可见性（2026-09-02）

反馈是「机器判定的信息显示不够突出，审核人很难发现该审核什么」。查真实 erpnext 本体
（866 个待复核，抽 282 个成员统计）后，问题不在样式而在**信息压根没被摊开**：

| 事实 | 占比 | 原来显示在哪 |
| --- | --- | --- |
| 启发式↔LLM 角色分歧 | 194/282（69%） | `role_reason` 散文的开头，只在右栏底部的项目符号里 |
| LLM 报告的证据缺口 | 184/282（65%） | 同一段散文的**末尾** |
| 分类器自我推翻（`reclassified_from`） | 51/282（18%） | 完全没显示 |
| 置信度 0.5（分歧的固定值） | 194/282 | 表格里没有这一列 |

而组头上最醒目的彩色标签是绿色的「证据充分」——它只表示得分离阈值远，却正好压在
一条「两个判定源打架 + 无注释无样例」的记录上，读起来像在说"这条没问题"。

### 改了什么

1. **判据结构化**（`utils/role.ts`）：`parseDisagreement` 把分歧句式拆回
   「LLM 判为 X（理由）/ 启发式判为 Y / 证据缺口 Z」；`reviewFlags` 把分歧、自我改判、
   证据缺口、低置信、反向信号统一成带轻重（alert/warn/info）的旗标。关系侧对称补
   `relationReviewFlags`（空动词 / 结构推断 / 低置信 / 自反）。
2. **共性归组头、例外归行内**（`summarizeFlags`）：29 张 InnoDB 系统表全是同一种分歧，
   逐行标红等于没标——组头写「机器分歧 29/29」，行内留白；11 张销售对象里只有 1 张是
   机器自我推翻，那一张的 chip 亮起来、行底染色、并被排到表首。「排除 N 个例外」(`X`)
   把它们剔出选择集，多数一次判掉。
3. **判定头条**：角色 + 把握 + 得分刻度（画出 2.0 阈值刻线）取代原来的小标签。组头取
   **组内最低**的把握/得分并写明是最低值——能不能整组确认取决于最弱那条。
4. **右栏先说"为什么要你看"**：分歧/缺口提到信号表之前，两个判定源并排陈列；被砍掉的
   「倾向」列以 28px 的箭头回归（没有方向的数字是读不出结论的）。
5. 顺手修了 `reasonClauses` 的分句：原来按「；」裸切，把嵌套括号里的半句切成独立条目，
   分歧理由因此显示成互不成句的碎片。现在按括号深度切。

中栏宽度是零和的：「邻居」列（118px）让给「机器存疑」，邻居移到右栏——那里放得下方向、
对象与关系动词；关系表的「结构/基数」也让位（结构本就是分组键，基数在右栏有）。
全组同旗时「机器存疑」列只剩破折号，列宽自动收窄把空间还给名称。

---

## 12. 队列本身太长：三条仲裁修复（2026-09-02）

界面把「该看什么」摊开之后，下一个问题是**队列压根不该这么长**。对 866 条待复核逐条
拆解（`backend/scripts/backfill_role_arbitration.py` 可复现）：

| 成因 | 条数 |
| --- | --- |
| 启发式↔LLM 分歧 | 668 |
| 启发式自己标的待复核 | 198（子表 106 + facet-B 69 + 其他 23） |

分歧的方向分布暴露了三个机制问题，**都不是判据不够，是仲裁把不是分歧的东西记成了分歧**。

### 12.1 确定性事实被推断结果覆盖（`evidence_builder._reclassify_bridge_to_object`）

Frappe 的 `parent`/`parenttype` 锚点是**源 schema 里读出来的事实**，`FrappeProfile.is_child_table`
识别得好好的，分类器也据此判成 bridge。但塌缩失败后，重判函数把 `is_child_table` 连同
软信号一起抑制掉重跑分类器，「单列业务主键 +2.0」独自越过阈值，明细行变回业务对象——
**把「这条关系我们表达不出来」当成了「那它就是个对象」**。

实测：337 张待复核表带子表锚点，`is_child_table` 信号在四类分歧里的触发次数是
0/271、0/112、0/219、0/47（全被抹掉了）。

修法：子表不参与重判，直接落 `data_table`（父表的明细），保留 `is_child_table` 证据。
`test_child_table_not_promoted_by_pk_signal` 两个方向都钉住：同样的信号，是子表就落
数据表，不是子表才走重判。

### 12.2 弃权被当成判定（`object_classifier` 兜底分支 + `_resolve_role`）

分类器证据不足时落到兜底句「信号不足，暂按业务对象保留，待人工确认」——那不是主张，
是弃权。但仲裁只做字符串比较 `llm_role == ot.table_role`，看到 `business_object ≠ technical`
就认定「两个独立源打架」。实测 271 条「业务对象→技术表」里有 161 条 score = 0.0，
`columns_priv` / `general_log` / `gtid_slave_pos` 这类连主键都没有。

修法：`ClassificationResult.abstained` 显式标记弃权（随 `role_signals` 落库），仲裁遇弃权
直接采纳 LLM 单源结论（置信度 0.7）。**唯一例外：LLM 说它是业务对象时仍要人点头**
——提升是会被发布的方向。

### 12.3 分歧不影响任何下游决定（`_resolve_role`）

`publish.py:433` 只提升 `table_role == "business_object"` 且已确认的对象。所以
data_table / technical / bridge 三者之间怎么判对产出零影响，却占了 278 条人工队列。

修法：仲裁先把角色归约到「是不是业务对象」这一个维度，只有跨越业务边界的分歧才置
`needs_review`；两源一致指向非业务时，顺手清掉启发式自己挂的复核标记（独立互证）。

### 效果与安全边界

存量重放（dry-run，未落库）：**866 → 305**，自动结案 561 条，其中 `technical` 444 /
`bridge` 117、**`business_object` 0 条**——这是这三条规则的硬约束：只清除复核标记，
绝不把任何对象自动提升为业务对象。剩下的 305 条是真正需要业务判断的：跨边界真分歧
（如 `event` 被 75 张表引用却是 MySQL 事件调度表）、facet-B 关联实体、81 个待确认的业务对象。

### 还没做的：阈值在这个源上没被标定

更深一层的原因是证据密度：**702/1033（68%）的表 `fk_in_degree = 0`**，启发式最强的拓扑
判据整体缺席；**636/1033（61%）有单列主键**（Frappe 每个 doctype 都有 `name` 列），光这
一条 +2.0 就顶满 2.0 的阈值——「是不是业务对象」实际退化成了「有没有主键」。两条正路：
按源的证据密度自适应权重，以及把 `FrappeProfile.inferred_fks` 的召回提上去。
