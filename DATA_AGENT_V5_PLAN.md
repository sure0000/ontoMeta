...
> 状态：**P0 已交付 + P0.5/P0.6 真实会话实测完成 + P1 收官**（T3/T3.2 交付，T2 延后至可查数据域）。F1/F2 已修。trace 已关回。
...
| 模块化 | 工具声明已拆；`_ReferenceResolver`/`_ObjectSnapshot`/`ChatBiService` 仍在 `chat_bi.py`（4291 行） | `chat_bi.py`、`chat_bi_tool_schemas.py` |
...
| **P1** | T2 预算据实调优 + T3 compaction 关键段保留 | 低 | 🟡 **T3/T3.2 ✅，T2 延后** |
| **P2** | T4 收尾拆 `chat_bi_references.py`（纯结构、零行为变化） | 中 | ⬜ |
| **P3** | T5 scout 多步取数链 + T6 trace 回放型 eval | 中 | ⬜ |
...
- [ ] T2.1 按实测调 `agent_history_char_budget` / `agent_result_sample_rows`；金标准：`avg_llm_calls` 不涨、golden 不变。
  **⚠ 延后（P1 收官时开放）**：`agent_history_char_budget` 可基于 P0.6 实测（budget=6000 时第 8 轮触发）先行调小试验；
  但 `agent_result_sample_rows` 必须靠**真实大结果（run_sql 执行）**才能定，而当前库无已验证可执行数据源（F3）——故 **T2 整体延后到接入一个「有可查数据 + 能长会话」的域**再做。
...
- [ ] T4.1 `_ObjectSnapshot` + `_ReferenceResolver` + `_loads_payload` 拆到 `chat_bi_references.py`，处理与 `ChatBiService` 的前引用（forward-ref 字符串保留）
- [ ] T4.2 `chat_bi.py` re-export，保持 `chat_bi._ReferenceResolver` 等对外符号与对象 identity 不变
- [ ] T4.3 全套件逐项不变（当前 1131 passed），行为零变化

### P3 详细任务

- [ ] T5.1 `query_scout_agent` 增多步预算，允许「profile→取样→改写 SQL」在隔离上下文里迭代；仍**不含 run_sql**
- [ ] T5.2 scout 返回结构不变（`candidate_sql`+要点），主 agent 执行链路不动
- [ ] T6.1 写 trace→golden 转换器：脱敏（去真实值/域名）后固化为 LLM-stub 用例
...
| T4 拆模块触发循环 import | `_ObjectSnapshot` 一并搬到 references 模块；`ChatBiService` 侧用 forward-ref 字符串引用 |
...
| T2 预算据实调参 | — | ⏳ 延后至可查数据域（F3） |

**真实会话实测硬数据**（P0.5/P0.6）：O4 隔离比 928–995×；O1 在 10 轮长会话第 8 轮触发、抑住上下文爆涨；
基线 `avg_llm_calls=2.6` 未回涨。全程不碰守卫，全库 **1149 passed**。
