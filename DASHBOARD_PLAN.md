...
**阶段 D3（对外与联动增强）— 🟡 部分已落地**
- ✅ **公开分享链接**（免登录只读）：public_token + 可选口令 + 有效期；`/api/public/data-apps/{token}`
  （豁免鉴权）；前端 `/public/apps/:token` 无壳只读页 + 口令输入 + 分享弹框。
- ✅ **对外数据解析图表 tile**：`/v1` 与 public 返回 datasets + widgets 渲染数据，看板完整渲染。
- ✅ **跨数据域图表库**：图表库面板“跨数据域”开关，可把不同域的图表拼到同一看板。
- ✅ **看板主题 + 移动端自适应**：浅/深色主题；ResponsiveGridLayout 断点（lg/md/sm/xs）自适应。
- ✅ **交叉过滤广播**：图表下钻作为看板级过滤广播，刷新看板内全部图表。
- ⛔ 看板级细粒度权限（私有/组织）：待用户/角色体系（当前仅管理 Token + 公开分享 + Cube 租户 RLS），延期。

**阶段 D4（治理与规模）— 🟡 部分已落地**
- ✅ **引用版本锁定**：发布看板时快照所引用图表定义；已发布看板的对外渲染用快照口径，
  图表后续编辑不影响已发布看板。
- ✅ **血缘**：`/api/data-apps/{id}/lineage` — 看板 → 图表/数据集 → 本体对象/字段；编辑器血缘弹框。
- ✅ **使用统计**：公开只读页访问计数 view_count。
- 🟡 图表/看板独立发版时间线、图表变更影响面分析：待续。
