...
- 写侧的顺序是固定的：`propose_*` → `draft_task` → `validate_task` → `confirm_task` → `execute_task` → `get_task_status`。前两步 editor 即可（无副作用），confirm/execute 需 publisher；**发什么角色的令牌就是“允不允许这个 agent 自动执行”的人为决定**，不要试图绕过它。
...
1. **结论**：一句话说明完成了什么、当前状态是什么。
2. **证据**：实际调用的 MCP 工具、关键真实 ID、数量、状态、是否截断。
3. **执行进度**（写侧才有）：列出每个阶段；执行中说“已受理，正在轮询”，不要把 accepted 写成 succeeded。
4. **下一步**：只给一项最有价值的。

不输出完整 Spec、整批对象、凭据、内部堆栈或无关的调用过程。用户要图时优先展示 `mermaid` / `vega_lite`，并说明它覆盖的是当前结果页还是样本；用了口径编译要给 `caliber_trace`，`fanout_note` / `warnings` 不得省略。
