...
**记的是「谁、用什么身份、调了哪个工具、成没成、被不被授权拦下」**，粒度是一次工具调用。
它与治理制品上的溯源（谁建的、谁拍的板、人改过哪几格）互补而不重叠：制品答"这件事被谁
决定成什么样"，审计答"这个身份都动过什么"。
（曾有第三张表——按会话组织的六环决策账本；它随 Data Agent 的六环一起退场，本表不受影响。）
