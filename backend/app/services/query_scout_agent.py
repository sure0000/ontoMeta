...
# V5 T5：多步取数链的预算。
#
# 原值 5 是「够走一趟直线」的量：定位 1 + 找 join 1 + profile 1 + 起草复查 1 + 收尾 1，
# 正好用光。于是**只要探路中间对不上一次**——列名不是想的那样、枚举值写法不同、
# join 键没声明——就没有余量改写后重查，被强制收尾交回一份没验完的草稿。
# 而没验完的候选 SQL 恰恰是最贵的产物：它看着能跑，主 agent 一执行才发现口径是错的。
# 放到 8 步，让「探→取样→按样例改写→再取」这条链跑得完；仍然封顶，超出照旧强制收尾
# （子 agent 的 LLM 调用单独计入 subagent_llm_calls，不污染主 avg_llm_calls）。
MAX_STEPS = 8
...
_SYSTEM_PROMPT = (
    ...
    "**按链路推进，不要一轮就交卷**：\n"
    "1. 定位：search_objects / search_logics 找到问题落在哪些对象、有没有现成口径。\n"
    "2. 取样：**每一个你打算写进 WHERE 的字段**都先 profile_values 看真实取值——"
    "枚举值的实际写法（是「已完成」还是 completed）、空值比例、取值基数，不看就是猜。\n"
    "3. 起草：按已经看到的**真实**列名与取值写候选 SQL。\n"
    "4. 校验改写：把草稿里还没验证过的假设逐个查实（列在不在、口径编译出来是什么、"
    "join 键有没有声明过）；对不上就**改写后重查**，不要把没验过的假设留在 SQL 里。\n"
    "5. 收尾：验证干净了再输出结论 JSON。\n"
    "纪律：\n"
    "1. 只用给你的探索工具；**不要执行 SQL**（你没有 run_sql，也不该有）。\n"
    "2. 写死过滤值前先 profile_values 看真实取值；跨对象先 find_join_path 拿声明过的关系。\n"
    "3. 没查过的列名、没 profile 过的取值一律不许写进 SQL。探不出就把 sql 设为空串、"
    "在 brief 里说清卡在哪一步——臆造一条「看着对」的 SQL 比交白卷代价高得多。\n"
    "4. 探完后**必须**只输出一个 JSON，不要任何其它文字：\n"
