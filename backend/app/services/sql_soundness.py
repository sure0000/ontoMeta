...
from app.services.ontology_projection import ObjView, OntologyProjection, RelView
...
def _other_is_many(r: RelView, m: ObjView) -> bool | None:
    """从对象 m 看，关系 r 的另一端是否为「多」端。未知基数返回 None。"""
    card = r.cardinality
    if card is None:
        return None
    # r 存的是 src→tgt 方向的基数。以 m 为「本端」换算另一端多重性。
    if m.name == r.src_obj:
...
def _group_by_columns(tree: exp.Expression) -> list[exp.Column]:
    ...
    for grp in tree.find_all(exp.Group):
        for e in grp.expressions:
            cols.extend(e.find_all(exp.Column))
    return cols
